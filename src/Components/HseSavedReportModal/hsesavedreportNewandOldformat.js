...
const HseSavedReportModal = ({
 ...
 const handleTableTypeChange = (newValues) => {
  ...
  // Merge in any newly created values
  setTableTypes((prev) => {
    const all = [...new Set([...prev, ...selectedValues])];
    return all;
  });

  // Update grouped observations safely
  setSafeGroupedObservations((prev) => {
    const updated = {};

    // Keep only those still selected
    selectedValues.forEach((val) => {
      updated[val] = prev[val] || [];
    });

    return updated;
  });

  // Filter all related observation arrays by selected table types
    ...
    <TableRow key={`new-row-${tableType}`}>
      ...
      <TableCell className="editable-cell" style={{ height: "100px" }}>
        <TextField
          ...
          placeholder="system_implementation"
